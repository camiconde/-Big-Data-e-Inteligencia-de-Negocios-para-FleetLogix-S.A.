...
-- Modelado Estrella en Snowflake - Avance 3
...
AUTO_SUSPEND = 60 -- Optimización de costos: se apaga tras 1 min de inactividad
AUTO_RESUME = TRUE; -- Encendido instantáneo automático al recibir consultas;

-- Crear la Base de Datos Analítica y el esquema para el Modelo Estrella
...
-- Dimensión Fecha: Fecha (Calendario completo con trimestres y festivos)
CREATE OR REPLACE TABLE dim_date (
    date_key INT PRIMARY KEY, -- Formato numérico AAAAMMDD (Ej: 20260603)
...
-- Dimensión 2: Tiempo (Análisis por horas y turnos de trabajo)
CREATE OR REPLACE TABLE dim_time (
    time_key INT PRIMARY KEY, -- Formato numérico HHMMSS o HHMM (Ej: 143000)
    hour INT,
    minute INT,
    second INT,
...
-- Dimensión 3: Vehículo (SCD Tipo 1 - Información técnica de la flota)
CREATE OR REPLACE TABLE dim_vehicle (
    vehicle_key INT PRIMARY KEY, -- Clave subrogada analítica
    vehicle_id INT NOT NULL,  -- ID operacional de PostgreSQL
...
-- Dimensión 4: Conductor (Estructurada para SCD Tipo 2 - Control Histórico de Desempeño)
CREATE OR REPLACE TABLE dim_driver (
    driver_key INT PRIMARY KEY,
    driver_id INT NOT NULL, -- ID operacional de PostgreSQL
...
-- Dimensión 5: Ruta (Geografía de origen y destino)
CREATE OR REPLACE TABLE dim_route (
    route_key INT PRIMARY KEY,
    route_id INT NOT NULL,  -- ID operacional de PostgreSQL
...
-- Dimensión 6: Cliente (Información comercial y demográfica)
CREATE OR REPLACE TABLE dim_customer (
    customer_key INT PRIMARY KEY,  -- ID operacional de PostgreSQL
...
-- =====================================================
-- 3. CREACIÓN DE LA TABLA CENTRAL DE HECHOS (FACT_DELIVERIES)
-- =====================================================

CREATE OR REPLACE TABLE fact_deliveries (
-- Claves foráneas vinculadas a las dimensiones (Llaves Subrogadas)
    ...
    -- Dimensiones Degeneradas (Mantiene la trazabilidad directa con la app móvil)
...
-- 4. CAPACIDAD DE TIME TRAVEL ("MÁQUINA DEL TIEMPO")
-- =====================================================
