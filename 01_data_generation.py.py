...
DB_CONFIG = {
    ...
    'user': 'postgres',
    'password': 'Vicen28',  # Cambiar por tu contraseña
    'port': 5432
}
...
class DataGenerator:
    ...
    def generate_vehicles(self, count=200):
        ...
        vehicles = []
        generated_plates = set()  # Conjunto para controlar duplicados en memoria
        ...
        while len(vehicles) < count:
            ...
            # Si la patente ya salió, nos saltamos esta iteración para generar otra
            if license_plate in generated_plates:
                continue
                
            generated_plates.add(license_plate)
        ...
        # Consulta estructurada para la inserción batch de vehículos
        query = """
    ...
    def generate_deliveries(self, count=400000):
        ...
        # RESOLUCIÓN DEL #TO DO 2#: Consulta oficial para extraer viajes y vincularlos a entregas
        self.cursor.execute("""
    ...
    def generate_maintenance(self, count=5000):
       ...
       # RESOLUCIÓN DEL #TO DO 3#: Consulta para extraer métricas de viajes y fechas por vehículo
        self.cursor.execute("""
            SELECT v.vehicle_id, v.vehicle_type, COUNT(t.trip_id) as trip_count, 
                   MIN(t.departure_datetime) as first_trip, MAX(t.departure_datetime) as last_trip
            FROM vehicles v
